...
const updateAssociationSchema = createAssociationSchema.partial().omit({ asset: true, part: true });
...
router.get('/', requireUser, async (req, res) => {
  try {
    const { asset, part, criticality, page = 1, limit = 50 } = req.query;

    const filter = {};

    // Multi-tenant Filter: Ensure we only show associations for asset in the active factory
    if (req.activeFactoryId) {
      // Get all asset IDs for this factory
      const factoryAsset = await Asset.find({ factory: req.activeFactoryId }).select('_id');
      const allowedAssetIds = factoryAsset.map(e => e._id);

      if (asset) {
        // Verify requested asset is in allowed list
        if (!allowedAssetIds.some(id => id.toString() === asset)) {
          // If requesting restricted asset, return empty
          return res.status(200).json({ success: true, associations: [], pagination: { page: 1, limit: parseInt(limit), total: 0, pages: 0 } });
        }
        filter.asset = asset;
      } else {
        // Constrain by all allowed asset
        filter.asset = { $in: allowedAssetIds };
      }
    } else if (asset) {
      filter.asset = asset;
    }
    ...
    const total = await AssetPart.countDocuments(filter);
...
router.get('/asset/:assetId', requireUser, async (req, res) => {
  try {
    ...
    const associations = await AssetPart.find({ asset: assetId })
      .populate('part', 'name partNumber category type currentStock minStock maxStock unitPrice supplier')
      .populate('changedBy', 'fullName email')
...
router.get('/part/:partId', requireUser, async (req, res) => {
  try {
    ...
    console.error('Error fetching part asset:', error);
    return res.status(500).json({ message: error.message });
  }
});
...
router.get('/reorder-alerts', requireUser, async (req, res) => {
  try {
    const alerts = await AssetPart.findPartsNeedingReorder(req.activeFactoryId);
...
router.post('/', requireUser, requireRole(['admin', 'maintenance_manager']), async (req, res) => {
  try {
    ...
    const association = new AssetPart({
      ...data,
    ...
    if (duplicateToSameType && asset.type) {
      try {
        ...
        console.error('Error duplicating to same type asset:', dupError);
    ...
    try {
      recalculatedMinMax = await AssetPartsService.recalculateMinMaxForPart(data.part);
    ...
    const populated = await AssetPart.findById(association._id)
    ...
    return res.status(201).json({
      ...
      message: duplicatedCount > 0
        ? `Association created and duplicated to ${duplicatedCount} asset(s) of the same type. Min/Max recalculated.`
...
router.patch('/:id', requireUser, requireRole(['admin', 'maintenance_manager']), async (req, res) => {
  try {
    ...
    if (association.asset.type) {
      try {
        ...
        for (const otherAsset of sameTypeAssets) {
          ...
          const otherAssoc = await AssetPart.findOne({
            asset: otherAsset._id,
        ...
        console.log(`Propagated changes to ${propagatedCount} asset(s) of the same type`);
      } catch (propError) {
    ...
    const populated = await AssetPart.findById(id)
      .populate('asset', 'model serialNumber')
    ...
    let recalculatedMinMax = null;
    try {
      recalculatedMinMax = await AssetPartsService.recalculateMinMaxForPart(association.part._id);
    ...
    return res.status(200).json({
      ...
      message: propagatedCount > 0
        ? `Association updated and propagated to ${propagatedCount} asset(s). Min/Max recalculated: ${recalculatedMinMax.minStock}/${recalculatedMinMax.maxStock}`
        : recalculatedMinMax
...
router.delete('/:id', requireUser, requireRole(['admin', 'maintenance_manager']), async (req, res) => {
  try {
    ...
    let recalculatedMinMax = null;
    try {
      recalculatedMinMax = await AssetPartsService.recalculateMinMaxForPart(partId);
