...
/**
 * AssetPart - Association entre équipement et pièce/consommable
...
schema.index({ asset: 1, part: 1 }, { unique: true });

// Index pour les requêtes fréquentes
schema.index({ asset: 1 });
schema.index({ part: 1 });
...
schema.statics.calculateGlobalStock = async function (partId) {
  ...
  return {
    ...
    assetCount: associations.length,
  ...
  for (const [partId, data] of partMap) {
    ...
    if (currentStock <= globalStock.globalReorderPoint) {
      results.push({
        ...
        assetCount: globalStock.assetCount
      });
    }
  }
...
const AssetPart = mongoose.model('AssetPart', schema);
