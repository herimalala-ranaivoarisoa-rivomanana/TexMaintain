...
const { ASSET_STATUSES, STATUS_METADATA } = require('./AssetStatusHistory');
...
const schema = new mongoose.Schema({
  ...
  assetClass: {
    type: mongoose.Schema.Types.ObjectId,
    ref: 'AssetClass',
    required: false, // Should be true after migration
    index: true
  },
  category: {
    type: mongoose.Schema.Types.ObjectId,
  ...
  usefulLifeYears: {
    type: Number,
    default: 10, // Standard approximation for industrial asset
    min: 1
  },
  salvageValue: {
    type: Number,
...
schema.methods.canTransitionTo = function (newStatus) {
  ...
  if (this.status === ASSET_STATUSES.SCRAPPED) return false;
...
module.exports = {
  Asset, // Renamed export
  Asset: Asset, // Alias for backward compat while refactoring
  ASSET_STATUSES,
  LEGACY_STATUS_MAP
};