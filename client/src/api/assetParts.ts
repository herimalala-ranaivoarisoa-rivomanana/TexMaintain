...
export interface UpdateAssetPartData {
  quantityPerMachine?: number
  replacementFrequencyPerYear?: number
  criticality?: Criticality
  machineImportance?: number
...
/**
 * Liste toutes les associations actif-pièce
 */
export const getAssetParts = async (params?: {
  asset?: string
  part?: string
  criticality?: Criticality
  page?: number
  ...
  const response = await api.get('/api/asset-parts', { params })
  return response.data
}

/**
 * Liste toutes les pièces associées à un actif
 */
export const getAssetPartsByAsset = async (assetId: string) => {
  const response = await api.get(`/api/asset-parts/asset/${assetId}`)
  return response.data
}

/**
 * Liste tous les actifs utilisant une pièce
 */
export const getAssetPartsByPart = async (partId: string) => {
  const response = await api.get(`/api/asset-parts/part/${partId}`)
  return response.data
}
...
export const calculateGlobalStock = async (partId: string): Promise<GlobalStockResponse> => {
  const response = await api.get(`/api/asset-parts/part/${partId}/global-stock`)
  return response.data
}
...
export const getReorderAlerts = async () => {
  const response = await api.get('/api/asset-parts/reorder-alerts')
  return response.data
}
 ...
 */
export const getAssetPart = async (id: string) => {
  const response = await api.get(`/api/asset-parts/${id}`)
  return response.data
}
 ...
 */
export const createAssetPart = async (data: CreateAssetPartData) => {
  const response = await api.post('/api/asset-parts', data)
  return response.data
}
 ...
 */
export const updateAssetPart = async (id: string, data: UpdateAssetPartData) => {
  const response = await api.patch(`/api/asset-parts/${id}`, data)
  return response.data
}
 ...
 */
export const deleteAssetPart = async (id: string) => {
  const response = await api.delete(`/api/asset-parts/${id}`)
  return response.data
}
