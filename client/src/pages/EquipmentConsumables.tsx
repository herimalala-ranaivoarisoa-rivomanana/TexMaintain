...
export function EquipmentConsumables() {
  ...
  const [selectedPartType] = useState<'consumable'>('consumable')
    ...
    const fetchData = async () => {
      ...
      try {
        const [equipmentPartsResponse, allPartsResponse] = await Promise.all([
          api.get(`/api/equipment/${id}/consumable`),
        ...
        toast({
          title: "Erreur",
          description: "Impossible de charger les consommables de l'équipement",
          variant: "destructive",
        })
      } finally {
        setLoading(false)
  ...
  const handleAddPart = async () => {
    ...
    try {
      setIsSaving(true)
      await api.post(`/api/equipment/${id}/consumable`, {
        part: selectedPart,
        quantity: partQuantity,
        changedBy: user?._id || "system"
      })
      
      toast({
        title: "Succès",
        description: "Consommable associé avec succès",
      })
      
      // Rafraîchir les données
      const response = await api.get(`/api/equipment/${id}/consumable`)
      ...
      console.error('Error adding consumable to equipment:', error)
      toast({
        title: "Erreur",
        description: "Impossible d'associer le consommable",
        variant: "destructive",
      })
    } finally {
      setIsSaving(false)
  ...
  const criticalPartsCount = filteredParts?.filter(ep => getStockStatus(ep.part) === 'critical').length
  const totalPendingOrders = filteredParts?.reduce((sum, ep) => sum + (ep.part.pendingQuantity || 0), 0)
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <CardContent className="p-4">
          <div className="relative">
            ...
            <Input
              placeholder="Rechercher des consommables..."
              value={searchTerm}
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <Card>
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-slate-600">Total Consommables</p>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-green-600">
                  ${filteredParts?.reduce((sum, ep) => sum + (ep.part.currentStock * ep.part.unitPrice), 0).toFixed(2)}
                </p>
              </div>
      ...
      {/* Message si aucun consommable */}
      {filteredParts?.length === 0 && (
        <Card>
          <CardContent className="p-12 text-center">
            <Droplet className="mx-auto h-12 w-12 text-slate-400 mb-4" />
            <h3 className="text-lg font-medium text-slate-900 mb-2">
              {equipmentParts?.length === 0 ? 'Aucun consommable associé' : 'Aucun consommable trouvé'}
            </h3>
            <p className="text-slate-600">
              {equipmentParts?.length === 0 
                ? 'Aucun consommable n\'est encore associé à cet équipement.'
      ...
      {/* Dialog Ajouter Consommable */}
      <Dialog open={isAddPartDialogOpen} onOpenChange={setIsAddPartDialogOpen}>
        <DialogContent className="sm:max-w-[500px] bg-white">
          <DialogHeader>
            <DialogTitle>Associer un Consommable</DialogTitle>
          </DialogHeader>
          <div className="grid gap-4 py-4">
            <div className="grid gap-2">
              <Label htmlFor="part">Consommable</Label>
