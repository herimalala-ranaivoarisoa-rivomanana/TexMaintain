...
export function EquipmentParts() {
  ...
  const [activeTab, setActiveTab] = useState<'parts' | 'consumables'>('parts')
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        console.log("ALLPARTS", allPartsResponse)
  ...
  const filteredParts = equipmentParts?.filter(ep => 
    !searchTerm || 
    !ep.part || // Skip if part is null
    ep.part.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
    ep.part.partNumber.toLowerCase().includes(searchTerm.toLowerCase())
  )

  // Séparer les pièces par type
  const partsOnly = filteredParts?.filter(ep => ep.part && ep.part.type === 'part') || []
  const consumablesOnly = filteredParts?.filter(ep => ep.part && ep.part.type === 'consumable') || []

  const PartList = ({ items }: { items: EquipmentPart[] }) => {
    const criticalPartsCount = items?.filter(ep => ep.part && getStockStatus(ep.part) === 'critical').length
    const totalPendingOrders = items?.reduce((sum, ep) => sum + (ep.part?.pendingQuantity || 0), 0)
    ...
    return (
      <>
          ...
          <Card>
            <CardContent className="p-4">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm text-slate-600">Total {activeTab === 'parts' ? 'Pièces' : 'Consommables'}</p>
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-green-600">
                    ${items?.reduce((sum, ep) => sum + ((ep.part?.currentStock || 0) * (ep.part?.unitPrice || 0)), 0).toFixed(2)}
                  </p>
                </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {items?.map((equipmentPart) => {
            // Skip if part is null (deleted or corrupted data)
            if (!equipmentPart.part) {
              return null;
            }
        ...
        {/* Message si aucune pièce/consommable */}
        {items?.length === 0 && (
          <Card>
            <CardContent className="p-12 text-center">
              <Package className="mx-auto h-12 w-12 text-slate-400 mb-4" />
              <h3 className="text-lg font-medium text-slate-900 mb-2">
                Aucun {activeTab === 'parts' ? 'pièce de rechange' : 'consommable'} associé{activeTab === 'parts' ? '' : ''}
              </h3>
              <p className="text-slate-600">
                {equipmentParts?.length === 0 
                  ? `Aucun ${activeTab === 'parts' ? 'pièce de rechange' : 'consommable'} n'est encore associé${activeTab === 'parts' ? '' : ''} à cet équipement.`
                  : `Aucun ${activeTab === 'parts' ? 'pièce de rechange' : 'consommable'} ne correspond à vos critères de recherche.`
                }
              </p>
            </CardContent>
          </Card>
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Onglets Pièces / Consommables */}
      <Tabs value={activeTab} onValueChange={(value) => setActiveTab(value as 'parts' | 'consumables')} className="w-full">
        <TabsList className="grid w-full grid-cols-2">
          <TabsTrigger value="parts" className="flex items-center gap-2">
            <Wrench className="h-4 w-4" />
            Pièces de Rechange ({partsOnly.length})
          </TabsTrigger>
          <TabsTrigger value="consumables" className="flex items-center gap-2">
            <Droplet className="h-4 w-4" />
            Consommables ({consumablesOnly.length})
          </TabsTrigger>
        </TabsList>

        <TabsContent value="parts" className="space-y-4">
          <PartList items={partsOnly} />
        </TabsContent>

        <TabsContent value="consumables" className="space-y-4">
          <PartList items={consumablesOnly} />
        </TabsContent>
      </Tabs>
        ...
        <DialogContent className="sm:max-w-[500px] bg-white">
          ...
          <div className="grid gap-4 py-4">
            <div className="grid gap-2">
              <Label htmlFor="part">Pièce de rechange</Label>
              <Select value={selectedPart} onValueChange={setSelectedPart}>
                ...
                <SelectContent>
                  {allParts
                    ...
                    .filter(part => !equipmentParts?.some(ep => ep.part && ep.part._id === part._id))
                    .map((part) => (
