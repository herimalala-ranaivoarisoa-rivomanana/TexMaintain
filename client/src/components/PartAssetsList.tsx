...
interface PartAssetsListProps {
  partId: string
}

export function PartAssetsList({ partId }: PartAssetsListProps) {
  const [associations, setAssociations] = useState<AssetPart[]>([])
  ...
  const fetchAssociations = async () => {
    try {
      setLoading(true)
      const response = await getAssetPartsByPart(partId)
  ...
  }).filter(assoc => assoc && (assoc.asset || assoc.asset))
        ...
        {associations.length === 0 ? (
          <div className="text-center py-8">
            ...
            <p className="text-sm text-slate-600">
              This part is not associated with any asset
            </p>
          </div>
        ) : (
          <div className="space-y-3">
            {sortedAssociations.map((assoc) => {
              ...
              const asset = assoc.asset || assoc.asset
