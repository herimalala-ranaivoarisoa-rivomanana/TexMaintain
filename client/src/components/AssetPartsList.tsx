...
import { AssetPartFormDialog } from './AssetPartFormDialog'
...
export function AssetPartsList({ assetId, type = 'part' }: AssetPartsListProps) {
  const [parts, setParts] = useState<AssetPart[]>([])
  const [loading, setLoading] = useState(true)
  const [isFormOpen, setIsFormOpen] = useState(false)
  const [editingPart, setEditingPart] = useState<AssetPart | null>(null)
  const [recordingReplacementFor, setRecordingReplacementFor] = useState<AssetPart | null>(null)
  ...
  const fetchParts = async () => {
    if (!assetId) {
      console.warn('[AssetPartsList] No asset ID provided')
      setLoading(false)
      return
    }
    ...
    try {
      ...
      const filteredParts = allParts.filter((assoc: AssetPart) => {
  ...
  const handleEdit = (part: AssetPart) => {
    setEditingPart(part)
    setIsFormOpen(true)
  }
  ...
  const handleRecordReplacement = (part: AssetPart) => {
    ...
    <>
          ...
          {parts.length === 0 ? (
            <div className="text-center py-8">
              ...
              <p className="text-sm text-slate-600">
                No {typeLabelSingular} associated with this asset
              </p>
      ...
      {recordingReplacementFor && (
        <RecordReplacementDialog
          assetPart={recordingReplacementFor}
