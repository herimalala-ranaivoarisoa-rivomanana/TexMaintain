...
export function AssetStatusHistory({ assetId, limit = 20 }: AssetStatusHistoryProps) {
  const id = assetId || '';
  ...
  const fetchHistory = async () => {
    if (!id) {
      console.warn('[AssetStatusHistory] No asset ID provided')
      setLoading(false)
      return
    }
    ...
    try {
      setLoading(true)
      console.log('[AssetStatusHistory] Fetching history for asset:', id)
      const response = await api.get(`/api/assets/${id}/status-history`, {
        params: { limit }
      })
      console.log('[AssetStatusHistory] Received:', response.data.total, 'entries')
