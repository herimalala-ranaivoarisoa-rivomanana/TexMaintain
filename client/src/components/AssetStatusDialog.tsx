...
import { changeAssetStatus, getAllowedTransitions } from '@/api/assets';
...
export function AssetStatusDialog({
  ...
  const fetchAllowedTransitions = async () => {
    try {
      ...
      const response = await getAllowedTransitions(id);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      toast({
        title: 'Success',
        description: `Asset status changed to ${getStatusLabelLocal(selectedStatus as string)}`
      });
      ...
      toast({
        title: 'Error',
        description: error.response?.data?.message || 'Failed to change asset status',
        variant: 'destructive'
      });
    } finally {
      setLoading(false);
