...
import { getStatusColor, getStatusLabel } from '@/types/asset'
...
export function AssetTimeline({ assetId, limit = 50 }: AssetTimelineProps) {
  const id = assetId || '';
  ...
  const fetchTimeline = async () => {
    if (!id) {
      console.warn('[AssetTimeline] No asset ID provided')
      setLoading(false)
      return
    }
    ...
    try {
      setLoading(true)
      console.log('[AssetTimeline] Fetching timeline for asset:', id)

      const eventTypes = filter === 'all' ? 'all' : filter === 'status' ? 'status' : filter === 'interventions' ? 'interventions' : 'parts'

      const response = await api.get(`/api/assets/${id}/timeline`, {
        params: { limit, eventTypes }
      })

      console.log('[AssetTimeline] Received:', response.data.total, 'events')
      ...
      toast({
        title: 'Error',
        description: 'Failed to load asset timeline',
        variant: 'destructive'
      })
    } finally {
      setLoading(false)
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              {event.data.previousStatus && (
                <>
                  <Badge className={`${getStatusColor(event.data.previousStatus as AssetStatus)} text-white mr-2`}>
                    {getStatusLabel(event.data.previousStatus as AssetStatus)}
                  </Badge>
                  <ArrowRight className="h-4 w-4 inline mx-1 text-slate-400" />
                </>
              )}
              <Badge className={`${getStatusColor(event.data.newStatus as AssetStatus)} text-white`}>
                {getStatusLabel(event.data.newStatus as AssetStatus)}
              </Badge>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="flex items-center gap-2 mt-1">
                ...
                <Badge className={`${event.data.priority === 'Critical' ? 'bg-red-500' :
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-lg font-semibold text-slate-900">Complete Asset Timeline</h3>
